...
class MatchMakingPlatformLogic:
    ...
    async def get_match_stats(
        ...
        if steam_id <= 0:
            request_data.error_msg = "完美平台查询失败，SteamId 无效"
            return None
        payload = {"matchId": match_id, "platform": "admin", "dataSource": "1"}
        timeout = aiohttp.ClientTimeout(total=15)
        async with session.post(
            url, json=payload, headers=self._common_headers(), timeout=timeout
        ) as resp:
    ...
    def _resolve_player(players, player_uuid: str | None, player_name: str):
        if player_uuid:
            player = MatchMakingPlatformLogic._resolve_player_by_uuid(
                players, player_uuid
            )
            if player is not None:
                return player
        return MatchMakingPlatformLogic._resolve_player_by_name(players, player_name)
    ...
    def _extract_player_data(player_raw, player_name: str, base_info) -> PlayerStats:
        team = MatchMakingPlatformLogic._resolve_player_team(player_raw, base_info)
        ...
        return PlayerStats(
            ...
            rating=float(player_raw.get("rating") or 0.0),
    ...
    async def get_premade_summary(
        ...
        for qq_id, player_info in user_data.items():
            platform_data = player_info.get("platform_data", {}).get("mm")
            ...
            if platform_data:
                ...
                if legacy_platform not in ("mm", "pw"):
                    continue
